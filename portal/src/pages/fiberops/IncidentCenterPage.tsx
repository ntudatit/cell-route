...
export function IncidentCenterPage(){const[data,setData]=useState<FiberIncident[]>([]);const[error,setError]=useState("");const[loading,setLoading]=useState(false);async function load(){setLoading(true);try{setError("");setData(await fiberOpsApi.incidents())}catch(e){setError(fiberErrorMessage(e))}finally{setLoading(false)}}useEffect(()=>{void load()},[]);return <AppLayout>
