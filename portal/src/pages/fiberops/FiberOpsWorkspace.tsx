...
export function FiberOpsWorkspace() {
  ...
  function select(tool: Tool) { setActive(tool); history.replaceState({}, "", `#${tool}`); window.dispatchEvent(new Event("feature-log-scope")); document.querySelector("#ops-workspace")?.scrollIntoView({ behavior: "smooth" }); }
...
function ReadinessTool() {
 ...
 try {
 setLoading(true); setError(""); try { setResult(await fiberOpsApi.readiness(invoice.trim())); } catch (cause) { featureOperation.fail(cause);  setError(fiberErrorMessage(cause)); } finally { setLoading(false); }
...
function ReconciliationTool() {
 ...
 const featureOperation = beginOperation(featureConsoleScope, 'action', 'run');
 try {
 setLoading(true); setError(""); try { setResult(await fiberOpsApi.reconcile(hash.trim())); } catch (cause) { featureOperation.fail(cause);  setError(fiberErrorMessage(cause)); } finally { setLoading(false); }
 } catch (featureError) { featureOperation.fail(featureError); throw featureError; } finally { featureOperation.complete(); }
}
...
function IncidentsTool() {
 ...
 try {
 setLoading(true); setError(""); try { setItems(await fiberOpsApi.incidents()); } catch (cause) { featureOperation.fail(cause);  setError(fiberErrorMessage(cause)); } finally { setLoading(false); }
 } catch (featureError) { featureOperation.fail(featureError); throw featureError; } finally { featureOperation.complete(); }
}
  useEffect(() => { void load(); }, []);
